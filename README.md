...
3. ブラウザで `http://localhost:8080/` にアクセスすると、Pixiv ログイン用のページが開きます。表示された「Pixivでログイン」ボタンから認証を行い、リダイレクト後のページ URL をコピーして戻ると、フロントエンドが自動的にコードを抽出してトークンを取得します。保存に成功するとリフレッシュトークンは `data/refresh_token.txt` に書き込まれ、次回以降は自動的に再利用されます。
